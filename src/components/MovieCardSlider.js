...
const MovieCardSlider = (props) => {
    ...
    return (
        <div
            className="md:w-[716px] w-full py-3 px-4 md:px-[42px] relative backdrop-blur-sm rounded-md duration-200 p-2 border-solid border-[1px] border-white/30"
            style={{
                backgroundImage: 'linear-gradient(106deg, rgba(255, 255, 255, 42%) 0%, rgba(51, 51, 51, 24%) 100%)',
                boxShadow: '0 4px 6px rgba(0, 0, 0, 0.1)',
            }}
        >
            <FaChevronLeft className="text-white/60 hover:text-white duration-300 cursor-pointer absolute left-3 top-[45%] hidden md:block" onClick={slideLeft} />
            <FaChevronRight className="text-white/60 hover:text-white duration-300 cursor-pointer absolute right-3 top-[45%] hidden md:block" onClick={slideRight} />
            <div className="">
